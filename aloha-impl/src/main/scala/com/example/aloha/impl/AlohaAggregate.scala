...
/**
  * This provides an event sourced behavior. It has a state, [[AlohaState]], which
  ...
  */
object AlohaBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[AlohaCommand]): Behavior[AlohaCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, AlohaEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[AlohaCommand, AlohaEvent, AlohaState](
        persistenceId = persistenceId,
        emptyState = AlohaState.initial,
...
/**
  ...
  */
case class AlohaState(message: String, timestamp: String) {
  def applyCommand(cmd: AlohaCommand): ReplyEffect[AlohaEvent, AlohaState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: AlohaEvent): AlohaState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[AlohaEvent, AlohaState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[AlohaEvent, AlohaState] =
    Effect
...
object AlohaState {
  ...
  /**
    ...
    */
  def initial: AlohaState = AlohaState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[AlohaCommand]("AlohaAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[AlohaState] = Json.format
}

/**
  * This interface defines all the events that the AlohaAggregate supports.
  */
sealed trait AlohaEvent extends AggregateEvent[AlohaEvent] {
  def aggregateTag: AggregateEventTag[AlohaEvent] = AlohaEvent.Tag
}

object AlohaEvent {
  val Tag: AggregateEventTag[AlohaEvent] = AggregateEventTag[AlohaEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends AlohaEvent
...
/**
  ...
  */
trait AlohaCommandSerializable

/**
  * This interface defines all the commands that the AlohaAggregate supports.
  */
sealed trait AlohaCommand
    extends AlohaCommandSerializable
...
/**
  ...
  * application loader.
  */
object AlohaSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[AlohaState],
