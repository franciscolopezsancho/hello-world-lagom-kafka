...
import com.example.aloha.api
import com.example.aloha.api.AlohaService
import akka.Done
import akka.NotUsed
...
/**
  * Implementation of the AlohaService.
  */
class AlohaServiceImpl(
  clusterSharding: ClusterSharding,
  persistentEntityRegistry: PersistentEntityRegistry
)(implicit ec: ExecutionContext)
  extends AlohaService {

  /**
    * Looks up the entity for the given ID.
    */
  private def entityRef(id: String): EntityRef[AlohaCommand] =
    clusterSharding.entityRefFor(AlohaState.typeKey, id)
  ...
  override def greetingsTopic(): Topic[api.GreetingMessageChanged] =
    TopicProducer.singleStreamWithOffset { fromOffset =>
      persistentEntityRegistry
        .eventStream(AlohaEvent.Tag, fromOffset)
  ...
  private def convertEvent(
    helloEvent: EventStreamElement[AlohaEvent]
