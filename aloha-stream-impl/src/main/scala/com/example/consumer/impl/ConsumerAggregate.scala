...
/**
  * This provides an event sourced behavior. It has a state, [[ConsumerState]], which
...
object ConsumerBehavior {
  ...
  /**
    ...
    */ 
  def create(entityContext: EntityContext[ConsumerCommand]): Behavior[ConsumerCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, ConsumerEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[ConsumerCommand, ConsumerEvent, ConsumerState](
        persistenceId = persistenceId,
        emptyState = ConsumerState.initial,
...
/**
  ...
  */
case class ConsumerState(message: String, timestamp: String) {
  def applyCommand(cmd: ConsumerCommand): ReplyEffect[ConsumerEvent, ConsumerState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: ConsumerEvent): ConsumerState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[ConsumerEvent, ConsumerState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[ConsumerEvent, ConsumerState] =
    Effect
...
object ConsumerState {
  ...
  /**
    ...
    */
  def initial: ConsumerState = ConsumerState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[ConsumerCommand]("ConsumerAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[ConsumerState] = Json.format
}

/**
  * This interface defines all the events that the ConsumerAggregate supports.
  */
sealed trait ConsumerEvent extends AggregateEvent[ConsumerEvent] {
  def aggregateTag: AggregateEventTag[ConsumerEvent] = ConsumerEvent.Tag
}

object ConsumerEvent {
  val Tag: AggregateEventTag[ConsumerEvent] = AggregateEventTag[ConsumerEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends ConsumerEvent
...
/**
  ...
  */
trait ConsumerCommandSerializable

/**
  * This interface defines all the commands that the ConsumerAggregate supports.
  */
sealed trait ConsumerCommand
    extends ConsumerCommandSerializable
...
/**
  ...
  * application loader.
  */
object ConsumerSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[ConsumerState],
